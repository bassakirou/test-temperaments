...
const translations = {
    fr: {
        ...
        welcomeText: "Bienvenue ! Ce test ludique te permettra de découvrir ton tempérament dominant parmi les 4 types : Sanguin, Colérique, Mélancolique, Flegmatique.",
        instructionText: "Pour chaque question, réponds de 1 (Pas du tout) à 5 (Tout à fait).",
        ...
        aiEnabled: "🤖 <strong>Analyse IA activée</strong> - Vous recevrez une analyse personnalisée détaillée !",
        aiDisabled: "📊 Analyse IA désactivée - Vous recevrez les résultats de base.",
        aiNotFound: "⚠️ Configuration IA non trouvée - Veuillez configurer config.js pour l'analyse IA.",
    ...
    en: {
        ...
        aiEnabled: "🤖 <strong>AI Analysis enabled</strong> - You will receive a detailed personalized analysis!",
        aiDisabled: "📊 AI Analysis disabled - You will receive basic results.",
        aiNotFound: "⚠️ AI Configuration not found - Please configure config.js for AI analysis.",
...
function showQuestion() {
    ...
    document.getElementById('progress').textContent = `${translations[currentLanguage].question} ${current + 1} ${translations[currentLanguage].of} ${questions.length}`;
...
function displayResults(dominant) {
    ...
    document.getElementById('temperament-title').innerHTML = 
        `${t.dominantTemperament} ${desc[dominant].emoji} ${desc[dominant].name}`;
    ...
    Object.keys(scores).forEach(temp => {
        ...
        scoreCard.style.borderColor = desc[temp].color;
        
        const percentage = Math.round((scores[temp] / (questions.length * 5)) * 100);
        
        scoreCard.innerHTML = `
            <h3 style="color: ${desc[temp].color};">${desc[temp].emoji} ${desc[temp].name}</h3>
            <div class="score-bar">
                <div class="score-fill" style="width: ${percentage}%; background-color: ${desc[temp].color};"></div>
            </div>
            <p>${percentage}%</p>
            <div class="traits">
                ${desc[temp].traits.map(trait => `<span class="trait">${trait}</span>`).join('')}
            </div>
        `;
        ...
        const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: `Tu es un expert en psychologie des tempéraments. Analyse les résultats du test et fournis une analyse personnalisée en ${currentLanguage === 'fr' ? 'français' : 'anglais'}.`
                    },
                    {
                        role: 'user',
